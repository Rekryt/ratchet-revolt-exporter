...
LUA
    ...
    private function valueKey(array $data): string {
        return implode(':', [$data['name'], $this->encodeLabelValues($data['labelValues']), 'value']);
    }
    ...
    public function collect(): array {
        try {
            ...
            return array_map(function (array $metric): MetricFamilySamples {
     ...
     * @throws StorageException
     */
    public function reconnectToServer() {
    ...
    private function connectToServer(): void {
        try {
            ...
            if ($this->options['persistent_connections'] !== false) {
                ...
                $connection_successful = $this->redis->connect(
                    $this->options['host'],
                    (int) $this->options['port'],
                    (float) $this->options['timeout']
                );
            ...
            if (!$connection_successful) {
                echo "Can't connect to Redis server" . PHP_EOL;
                //throw new StorageException("Can't connect to Redis server", 0);
            }
        } catch (\RedisException $e) {
            echo "Can't connect to Redis server: " . $e->getMessage() . PHP_EOL;
    ...
    private function collectSummaries(): array {
        ...
        foreach ($keys as $metaKeyWithPrefix) {
            ...
            foreach ($values as $valueKeyWithPrefix) {
                ...
                $samples = [];
                $sampleValues = $this->redis->keys(
                    $summaryKey . ':' . $metaData['name'] . ':' . $encodedLabelValues . ':value:*'
                );
